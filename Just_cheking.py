...
#trail_df = pd.read_csv('even_smaller_data.txt', header=None)
trail_df = pd.read_csv('C:/Users/adamg/361_A1/trail.csv')
...
trail_df = oneHotEncoder(trail_df)
print(trail_df)
...
trail_df = replaceWithBinary(trail_df) 

trail_df = trail_df.astype(int)
print(trail_df)
...
class DT_Node:
    def __init__(self, dataframe,feature = None, threshold=None, parent = None, left=None, right=None, depth=0):
        ...
        self.probability = self.df['OK'].mean()
...
other_root = DT_Node(trail_df)
tree = train_tree(other_root,5)

def print_tree(node, indent=''):
    if node == None:
        return
    if node.prediction == 0:
        poison_status = 'Not OK'
    else:
        poison_status = 'OK'
...
pred = make_predictions(tree, trail_df)
Accuracy, Percision, Recall  = calculate_performance(trail_df, pred)
...
def calculate_rates(df, p_hat, threshold):
    ...
    y_true = df['OK'].astype(int).to_numpy()
...
p_pred = give_probabilities(tree, trail_df)
print("p_preds are:")
print(p_pred)
tprs, fprs = draw_roc_curve(trail_df, p_pred)
print("TPRs:")
print(tprs)
print("FPRs:")
print(fprs)
